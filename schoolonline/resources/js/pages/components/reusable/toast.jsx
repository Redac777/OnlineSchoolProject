...
const Toast = ({ type, message}) => {
  ...
  const toastClasses = `flex
  absolute z-10 fixed left-1/2 top-24 transform transition-all duration-700
  items-center gap-2 w-full max-w-md p-2.5 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800
`;
    ...
    <div className={toastClasses} role="alert">
      {type === 'success' && (
        <div className="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-green-500 bg-green-100 rounded-lg dark:bg-green-800 dark:text-green-200">
          ...
          <span className="sr-only px-3">Check icon</span>
        </div>
      )}
      {type === 'danger' && (
