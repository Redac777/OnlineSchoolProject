...
const StudentSideBar = ({user,handleSidebarClick,renderedComponent,school,year}) => {

    const [level,setLevel] = useState(null);
    const [classe,setClasse] = useState(null);
    useEffect(() => {
        axios.get(`/studentLevelAndClass`).then((response) => {
        ...
        <div className={`print:hidden bg-main-bg flex flex-col border-dashed ${user.lang=="عربي" ? "border-l" : "border-r"} h-screen`}>
            ...
            <div className={`mt-10 mx-6 flex ${user.lang=="عربي" && "justify-end"}  text-sm items-center py-3 px-4 rounded-lg bg-gray-200 gap-5`}>
                {user.lang=="عربي" ? (
                    ...
                    <h3 className="font-roboto text-gray-700 font-black">{user.lastName + " " + user.firstName}</h3>
                    <h3 className='font-semibold text-xsm'>Level : {levelName} ~ Class : {className}</h3>
                    </div>
                    </>
                )}
            </div>
            ...
            <ul className="mt-6 flex flex-col gap-1">
                ...
                {/* Posts */}
                <li className={`flex items-center ${user.lang=="عربي" ? "justify-end" : "justify-start"} gap-4 mx-6 cursor-pointer py-3 px-4 rounded-lg hover:bg-slate-300 ${renderedComponent==="ListStudentPosts" ? "bg-slate-300":""}`}>
                {user.lang=="عربي" ? (
                    <>
                    <a className={`text-sm ${(renderedComponent==="ListStudentPosts")? "text-slate-800 font-bold" : "text-sideBarITemsColor font-medium"}`} onClick={()=>{handleSidebarClick("ListStudentPosts")}}>
                    منشورات                          </a>
                    <img src={postsIcon} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListStudentPosts") ? "bg-slate-800" : ""}`} />
                    </>
                ): (
                    <>
                    <img src={postsIcon} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListStudentPosts") ? "bg-slate-800" : ""}`} />
                ...
                {/* Absences */}
                <li className={`flex items-center ${user.lang=="عربي" ? "justify-end" : "justify-start"} gap-4 mx-6 cursor-pointer py-3 px-4 rounded-lg hover:bg-slate-300 ${renderedComponent==="ListStudentAbsences" ? "bg-slate-300":""}`}>
                {user.lang=="عربي" ? (
                    <>
                    <a className={`text-sm ${(renderedComponent==="ListStudentAbsences")? "text-slate-800 font-bold" : "text-sideBarITemsColor font-medium"}`} onClick={()=>{handleSidebarClick("ListStudentAbsences",false)}}>
                    غيابات                            </a>
                    <img src={level} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListStudentAbsences") ? "bg-slate-800" : ""}`} />
                    </>
                ): (
                    <>
                    <img src={level} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListStudentAbsences") ? "bg-slate-800" : ""}`} />
