...
const TeacherSideBar = ({user,handleSidebarClick,renderedComponent,school}) => {
    return (
        <div className={`print:hidden bg-main-bg flex flex-col border-dashed ${user.lang=="عربي" ? "border-l" : "border-r"} h-screen`}>
            ...
            <ul className="mt-6 flex flex-col gap-1">
                ...
                {/* Absences */}
                <li className={`flex items-center ${user.lang=="عربي" ? "justify-end" : "justify-start"} gap-4 mx-6 cursor-pointer py-3 px-4 rounded-lg hover:bg-slate-300 ${renderedComponent==="ListAbsenceReports" ? "bg-slate-300":""}`}>
                {user.lang=="عربي" ? (
                    <>
                    <a className={`text-sm ${(renderedComponent==="ListAbsenceReports")? "text-slate-800 font-bold" : "text-sideBarITemsColor font-medium"}`} onClick={()=>{handleSidebarClick("ListAbsenceReports",false)}}>
                    غيابات                            </a>
                    <img src={level} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListAbsenceReports") ? "bg-slate-800" : ""}`} />
                    </>
                ): (
                    <>
                    <img src={level} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListAbsenceReports") ? "bg-slate-800" : ""}`} />
                    <a className={`text-sm ${(renderedComponent==="ListAbsenceReports")? "text-slate-800 font-bold" : "text-sideBarITemsColor font-medium"}`} onClick={()=>{handleSidebarClick("ListAbsenceReports",false)}}>
                    {user.lang=="eng" ? 'Absences' : 'Absences'}
                    </a>
                    </>
                )}
                </li>
                ...
                {/* Grades */}
                <li className={`flex items-center ${user.lang=="عربي" ? "justify-end" : "justify-start"} gap-4 mx-6 cursor-pointer py-3 px-4 rounded-lg hover:bg-slate-300 ${renderedComponent==="ListTeacherStudents" ? "bg-slate-300":""}`}>
                {user.lang=="عربي" ? (
                    <>
                    <a className={`text-sm ${(renderedComponent==="ListTeacherStudents")? "text-slate-800 font-bold" : "text-sideBarITemsColor font-medium"}`} onClick={()=>{handleSidebarClick("ListTeacherStudents")}}>
                    درجات                       </a>
                    <img src={classIcon} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListTeacherStudents") ? "bg-slate-800" : ""}`} />
                    </>
                ): (
                    <>
                    <img src={classIcon} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListTeacherStudents") ? "bg-slate-800" : ""}`} />
                ...
                {/* Posts */}
                <li className={`flex items-center ${user.lang=="عربي" ? "justify-end" : "justify-start"} gap-4 mx-6 cursor-pointer py-3 px-4 rounded-lg hover:bg-slate-300 ${renderedComponent==="ListTeacherPosts" ? "bg-slate-300":""}`}>
                {user.lang=="عربي" ? (
                    <>
                    <a className={`text-sm ${(renderedComponent==="ListTeacherPosts")? "text-slate-800 font-bold" : "text-sideBarITemsColor font-medium"}`} onClick={()=>{handleSidebarClick("ListTeacherPosts")}}>
                    منشورات                          </a>
                    <img src={postsIcon} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListTeacherPosts") ? "bg-slate-800" : ""}`} />
                    </>
                ): (
                    <>
                    <img src={postsIcon} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListTeacherPosts") ? "bg-slate-800" : ""}`} />
