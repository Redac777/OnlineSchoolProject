...
const AdministrationDepartmentSideBar = ({user,handleSidebarClick,renderedComponent,school}) => {
    return (
        <div className={`print:hidden bg-main-bg flex flex-col border-dashed ${user.lang=="عربي" ? "border-l" : "border-r"} h-screen`}>
            ...
            <ul className="mt-6 flex flex-col gap-1">
                ...
                {/* Absences */}
                <li className={`flex items-center ${user.lang=="عربي" ? "justify-end" : "justify-start"} gap-4 mx-6 cursor-pointer py-3 px-4 rounded-lg hover:bg-slate-300 ${renderedComponent==="ListTeacherAbsenceReports" ? "bg-slate-300":""}`}>
                {user.lang=="عربي" ? (
                    <>
                    <a className={`text-sm ${(renderedComponent==="ListTeacherAbsenceReports")? "text-slate-800 font-bold" : "text-sideBarITemsColor font-medium"}`} onClick={()=>{handleSidebarClick("ListTeacherAbsenceReports",false)}}>
                    غيابات                            </a>
                    <img src={level} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListTeacherAbsenceReports") ? "bg-slate-800" : ""}`} />
                    </>
                ): (
                    <>
                    <img src={level} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListTeacherAbsenceReports") ? "bg-slate-800" : ""}`} />
                ...
                {/* Posts */}
                <li className={`flex items-center ${user.lang=="عربي" ? "justify-end" : "justify-start"} gap-4 mx-6 cursor-pointer py-3 px-4 rounded-lg hover:bg-slate-300 ${renderedComponent==="ListAdministrationPosts" ? "bg-slate-300":""}`}>
                {user.lang=="عربي" ? (
                    <>
                    <a className={`text-sm ${(renderedComponent==="ListAdministrationPosts")? "text-slate-800 font-bold" : "text-sideBarITemsColor font-medium"}`} onClick={()=>{handleSidebarClick("ListAdministrationPosts")}}>
                    منشورات                          </a>
                    <img src={postsIcon} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListAdministrationPosts") ? "bg-slate-800" : ""}`} />
                    </>
                ): (
                    <>
                    <img src={postsIcon} className={`w-6 p-1 rounded-full ${(renderedComponent=="ListAdministrationPosts") ? "bg-slate-800" : ""}`} />
...
export default AdministrationDepartmentSideBar;
