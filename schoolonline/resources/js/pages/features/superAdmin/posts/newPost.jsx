...
const NewPost = ({user,year,schoolToEdit,handleMainClick }) => {
  ...
  useEffect(() => {
    // Récupérer la liste des écoles en fonction de l'année


    axios.get(`/listSchools?selectedYear=${year}`).then((response) => {
  ...
  const handlePostSubmit = async (e) => {
    ...
    const storeType = ["Schools"];
    const recip = recipient === "all" ? JSON.stringify(["AllSchools"]) : JSON.stringify(storeType.concat(selectedSchools.map((school) => school.value)));
  ...
  const handleSchoolCheckboxChange = (schoolId) => {
    if (selectedSchools.includes(schoolId)) {
      // Si l'école est déjà sélectionnée, la retirer de la liste
        ...
        <img className='w-10 bg-slate-700 rounded-full mr-4 mx-7' src={`http://localhost:8000/storage/${user.userProfil}`}/>
          ...
          <div className="mx-4">
            <input
              ...
              onChange={() => setRecipient("specific")}
            />
