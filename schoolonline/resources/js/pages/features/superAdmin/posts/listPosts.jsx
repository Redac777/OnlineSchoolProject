...
const ListPosts = ({handleMainClick,year,schoolToEdit}) => {
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        if (selectedSchool) {
            ...
            axios.get(`/listPosts?school_id=${selectedSchool.value}&selectedYear=${year}&school=${schoolToEdit.id}&addvalue=${addvalue}`).then((response) => {
              setPosts(response.data);
            });
          }
         else {
          // Fetch all subjects without schoolfilter
      ...
      const handleFilterByCategory = (selectedOption) => {
        ...
        setSelectedSchool(selectedOption); // Mettre à jour la catégorie sélectionné
    };

      const handleClearFilter = () => {
        setSelectedSchool(null); // Réinitialiser la catégorie sélectionnée
    ...
    return(
        ...
        <div className="flex flex-col justify-center items-center">
            <div className="w-full flex justify-center mt-4 gap-4">
                <button className={` px-4 py-2 rounded-lg text-slate-800 font-medium hover:bg-gray-400 ${addvalue=="authUser" ? "bg-slate-400" : "bg-gray-300"}`} type="button" onClick={()=>{setAddvalue("authUser")}}>My posts</button>
                <button className= {`bg-gray-300 px-4 py-2 rounded-lg text-slate-800 font-medium hover:bg-gray-400 ${addvalue=="other" ? "bg-slate-400" : "bg-gray-300"}`} type="button" onClick={()=>{setAddvalue("other")}}>School posts</button>
            </div>
