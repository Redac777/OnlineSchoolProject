...
const NewAdministrationPost = ({ user, year, schoolToEdit, handleMainClick }) => {
  // ...
  ...
  const handlePostSubmit = async (e) => {
    ...
    if(recipientType === "all") {
        recipientHandleValue = JSON.stringify(["administration","all"]);
    }
    else if(recipientType === "specific"){
        if(recipientValues=="teacher"){
            if(selectedTeachers.length === 0) recipientHandleValue = JSON.stringify(["administration","allTeachers"]);
            else {
                const teacherIds = selectedTeachers.map((teacher) => teacher.userId);
                recipientHandleValue = JSON.stringify(["administration"].concat([recipientValues[0]].concat(teacherIds)));
            }
        }
        else{
            if(selectedLevels.length === 0) recipientHandleValue = JSON.stringify(["administration","allLevels"]);
            else {
                if(selectedClasses.length === 0)
                {
                    const levelIds = selectedLevels.map((level) => level.value);
                    const storeType = "allLevelClasses";
                    recipientHandleValue = JSON.stringify(["administration"].concat([storeType].concat(levelIds)));
                }

                else {
                    const classesIds = selectedClasses.map((classe) => classe.value);
                    recipientHandleValue = (JSON.stringify(["administration"].concat([recipientValues[0]].concat(classesIds))));
                }
            }
        }
    }
    ...
    try {
      ...
      handleAlert("success");
      setTimeout(() => {
      handleMainClick("ListAdministrationPosts");
    }, 1500);
...
export default NewAdministrationPost;
