...
const ListLevels = ({handleMainClick,filtre,year,schoolToEdit}) => {
    const [levels,setLevels] = useState([]);
    const [searchQuery, setSearchQuery] = useState('');
    const [selectedCategory, setSelectedCategory] = useState(null); // Nouveau state pour la catégorie sélectionnée
    ...
    useEffect(() => {
      ...
      const handleFilterByCategory = (selectedOption) => {
        ...
        setSelectedCategory(selectedOption); // Mettre à jour la catégorie sélectionnée
      };

      const handleDeleteLevel = (id) => {
        const confirmation = confirm("Please confirm delete action");
        if(confirmation){
        axios.delete(`/deletedLevel/${id}`)
          .then((response) => {
            if(response.data.message =="Level not found")
            handleAlert("danger");
            else if(response.data.message=="Cannot delete this level")
            handleAlert("warning");
            else{
            handleAlert("success");
            setLevels((prevLevels) => prevLevels.filter((level) => level.id !== id));
        }
            // Refresh your level list or perform other actions as needed
          })
          .catch((error) => {
            handleAlert("danger");
          });
    ...
    const listLevels = currentLevels.filter((level) =>
    level.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
    (!isFilterActive || !selectedCategory || level.category === selectedCategory.value)).map(level => {
    return(
        <tr class="bg-white border-b">
            ...
            <td className="px-6 py-3">
                <div className="flex w-full h-full items-center">
                    <a href="#" className="font-medium text-footerColor hover:underline" onClick={()=>handleMainClick("ListClasses",level)}>Classes</a>
                    <a href="#" className="font-medium text-violet-600 hover:underline pl-5" onClick={()=>handleMainClick("EditLevel",level)}>Edit</a>
                    <a href="#" className="font-medium text-yellow-600 hover:underline pl-5" onClick={()=>{handleDeleteLevel(level.id)}}>Delete</a>
                </div>
            </td>
        </tr>
    )});
...
const renderPagination = () => {
    ...
    return(
        <div className="w-full flex flex-col justify-center items-center -mt-12">
            {showSuccessToast && <Toast type="success" message="Level deleted successfully." />}
            {showDangerToast && <Toast type="danger" message="Error deleting level" />}
            {showDangerToast && <Toast type="warning" message="Cannot delete this level !!" />}
        <div className="flex justify-start items-center w-5/6">
        <div className="w-1/4">
        <Button svgContent={plusButtonSvg} buttonText="Add" type="button" onClick={()=>{handleMainClick("NewLevel")}}/>
        </div>
        <div className="w-3/4">
        <form>
