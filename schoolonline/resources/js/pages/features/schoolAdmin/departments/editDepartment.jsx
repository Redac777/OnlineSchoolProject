...
const EditDepartment = ({handleMainClick,department,schoolToEdit,year}) =>{
    const [name,setName] = useState(department?.name || "");
    const [lastName, setLastName] = useState(department?.user.lastName || "");
    const [firstName, setFirstName] = useState(department?.user.firstName || "");
    const [email, setEmail] = useState(department?.user.email || "");
    const [phone,setPhone] = useState(department?.user.phone || "");
    const [password,setPassword] = useState(department?.user.password || "");
    ...
    const [showWarningToast, setShowWarningToast] = useState(false);
      useEffect(() => {
          ...
          const handleSubmit = async (event) => {
            ...
            if(department.user.lastName != lastName)
                generatedRandomCode = generateRandomCode();
                ...
                // const code = generatedRandomCode!=""? generatedRandomCode : department.user.code;
                // const username = generatedRandomCode!=""? generatedRandomCode : department.user.username;
                // const password = generatedRandomCode!=""? generatedRandomCode : department.user.password;
                // console.log("code : " + code);
                // console.log("username : " + username);
                // console.log("password : " + password);
        try {
            if(generatedRandomCode==""){
                response = await axios.put(`/update-department/${department.id}`, {
                    name : name,
                  });
