...
const ListClasses = ({handleMainClick,filtre,level,year,schoolToEdit}) => {
    const [classes,setClasses] = useState([]);
    const [searchQuery, setSearchQuery] = useState('');
    const [selectedLevel, setSelectedLevel] = useState(null); // Nouveau state pour la catégorie sélectionnée
    const [existingLevel ,setExistingLevel] = useState(level?.id || null);
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        if (selectedLevel) {
            ...
            axios.get(`/listClasses?level_id=${existingLevel}&selectedYear=${year}&school=${schoolToEdit.id}`).then((response) => {
                setClasses(response.data);
              });
        }
         else {
    ...
    useEffect(() => {
      ...
      const handleDeleteClass = (id) => {
        ...
        if(confirmation){
        axios.delete(`/deletedClass/${id}`)
          .then((response) => {
            if(response.data.message =="Class not found")
            handleAlert("danger");
            else if(response.data.message=="Cannot delete this class")
            handleAlert("warning");
            else{
            handleAlert("success");
      ...
      let currentPosts = [];
      if(classes.length > 0)
      currentPosts =classes.slice(indexOfFirstPost, indexOfLastPost);

    const listClasses = currentPosts.map(classe => {
    return(
        <tr classe="bg-white border-b">
            ...
            <td className="px-6 py-3">
                <div className="flex w-full h-full items-center">
                    <a href="#" className="font-medium text-footerColor hover:underline" onClick={()=>handleMainClick("ListStudents",classe)}>Students</a>
                    <a href="#" className="font-medium text-violet-600 hover:underline pl-5" onClick={()=>handleMainClick("EditClass",classe)}>Edit</a>
                    <a href="#" className="font-medium text-yellow-600 hover:underline pl-5" onClick={()=>{handleDeleteClass(classe.id)}}>Delete</a>
                    <a href="#" className="font-medium text-blue-600 hover:underline pl-5" onClick={()=>{handleMainClick("UpgradeClass",classe)}}>Upgrade</a>
                </div>
            </td>
        </tr>
    )});
    const options = levels.map((level) => ({
      ...
      const totalPages = Math.ceil(classes.length / postsPerPage);
...
const renderPagination = () => {
    ...
    return(
        <div className="w-full flex flex-col justify-center items-center -mt-12">
            {showSuccessToast && <Toast type="success" message="class deleted successfully." />}
            {showDangerToast && <Toast type="danger" message="Error deleting class" />}
            {showDangerToast && <Toast type="warning" message="Cannot delete this class !!" />}
        <div className="flex justify-start items-center w-5/6">
        <div className="w-1/4">
        <Button svgContent={plusButtonSvg} buttonText="Add" type="button" onClick={()=>{handleMainClick("NewClass")}}/>
        </div>
        <div className="w-3/4">
        <form>
        <div classe="relative">
