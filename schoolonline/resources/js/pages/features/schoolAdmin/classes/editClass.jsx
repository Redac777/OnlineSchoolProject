...
const EditClass = ({handleMainClick,classe}) =>{
    ...
    const [showWarningToast, setShowWarningToast] = useState(false);
      ...
      useEffect(() => {
        ...
        try {
          const response = await axios.put(`/update-class/${classe.id}`, {
            name : name,
          });
          if (response.data.message === 'Class updated successfully') {
          // Réinitialiser les champs et afficher un message de succès
          handleAlert("success");
          setTimeout(() => {
          setName('');
          handleMainClick("ListClasses");
        }, 1500);
        }
          else{
            handleAlert("danger");
    ...
    return(
        <div className="w-full flex flex-col justify-center">
            <div>
                {showSuccessToast && <Toast type="success" message="Class updated successfully." />}
                {showDangerToast && <Toast type="danger" message="Error updating class" />}
            </div>
            <div className="w-full flex justify-center items-center">
                <h2 className="text-gray-700 font-medium text-xl">Edit Class</h2>
            </div>
            <div className="w-full flex justify-center items-center">
