...
const ListSchoolPosts = ({handleMainClick,year,schoolToEdit}) => {
    ...
    const plusButtonSvg = (
      ...
      useEffect(() => {
        ...
        const fetchData = async () => {
          try {
            const response = await fetch(`/listDepartments?selectedYear=${year}&school=${schoolToEdit.id}`);
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        if (selectedDepartment) {
            ...
            axios.get(`/listPosts?department_id=${selectedDepartment.userId}&selectedYear=${year}&school=${schoolToEdit.id}&addvalue=${addvalue}`).then((response) => {
              setPosts(response.data);
            });
          }
        else if(selectedTeacher){

            axios.get(`/listPosts?teacher_id=${selectedTeacher.value}&selectedYear=${year}&school=${schoolToEdit.id}&addvalue=${addvalue}`).then((response) => {
              setPosts(response.data);
            });
        }
         else {
      ...
      }, [selectedDepartment,selectedTeacher,year,addvalue]);
      ...
      const handleFilterByCategory = (selectedOption,othervalue) => {
        setIsFilterActive(true);
        if(othervalue=="teacher"){
            setSelectedTeacher(selectedOption);
            setSelectedDepartment(null);
        }
        else if(othervalue=="department") {
      ...
      const handleClearFilter = () => {
        setSelectedDepartment(null); // Réinitialiser la catégorie sélectionnée
        setSelectedTeacher(null);
        ...
        return(
            <div className="mb-6">
                ...
                >
                    ...
                    <span className="font-medium">{post.user.lastName + " " +post.user.firstName}</span>
                    <span>{date + " "+time}</span>
                    </div>
                    <div className="flex justify-center">
                    <a href="#" className="font-medium text-slate-800 hover:underline  pl-5" onClick={()=>{handleDeletePost(post.id)}}>delete</a>
                    </div>
                    </div>
                </a>
                ):(
                    ...
                    key={index}
                    href={isImage(post.file.file_path) ? post.file.file_path : "#"}
                    className="flex flex-col justify-center items-center space-y-4 p-4 leading-normal bg-white border border-gray-200 rounded-lg shadow w-96 h-48 mt-6"                    >
...
const renderPagination = () => {
    ...
    return(
        ...
        <div className="w-1/4">
        <Button svgContent={plusButtonSvg} buttonText="Add" type="button" onClick={()=>{handleMainClick("NewSchoolPost")}}/>
        </div>
        <div className="w-3/4">
            <form>
            <div classe="relative">
            ...
            <div className="flex justify-center items-center">
                ...
                isSearchable={false}
                onChange={(selectedOption) => handleFilterByCategory(selectedOption, "department")}
        ...
        <div className="flex flex-col justify-center items-center">
            <div className="w-full flex justify-center mt-4 gap-4">
                ...
                <button className= {`bg-gray-300 px-4 py-2 rounded-lg text-slate-800 font-medium hover:bg-gray-400 ${addvalue=="superadmin" ? "bg-slate-400" : "bg-gray-300"}`} type="button" onClick={()=>{
                    setShowSelectDepartment(false);
                    setShowSelectTeacher(false);
                    setAddvalue("superadmin")}}>Super Admin</button>
                <button className= {`bg-gray-300 px-4 py-2 rounded-lg text-slate-800 font-medium hover:bg-gray-400 ${addvalue=="department" ? "bg-slate-400" : "bg-gray-300"}`} type="button" onClick={()=>{
                    setShowSelectDepartment(true);
                    setShowSelectTeacher(false);
                    setAddvalue("department")}}>Departments</button>
                    <button className= {`bg-gray-300 px-4 py-2 rounded-lg text-slate-800 font-medium hover:bg-gray-400 ${addvalue=="teacher" ? "bg-slate-400" : "bg-gray-300"}`} type="button" onClick={()=>{
                    setShowSelectDepartment(false);
                    setShowSelectTeacher(true);
                    setAddvalue("teacher")}}>Teachers</button>
            </div>
...
export default ListSchoolPosts;
